...
def mcp_message_transport_span() -> "Span | None":
    ...
    of ``error.*`` stamping from another task. A finished span keeps a valid context
    forever, so ``is_recordable_span`` alone would happily hand back one the SDK
    then refuses to write. The POST carrying a ``tools/call`` stays open until the
    result is written, so it is recording for the life of the call; a notification
    POST can answer first, and this returns ``None`` for it rather than writing into
    the void.
    """
    span = _mcp_message_transport_span.get()
    return span if is_recordable_span(span) and span.is_recording() else None  # type: ignore[union-attr]
...
def _mcp_transport_span_context() -> "SpanContext | None":
    ...
    if is_recordable_span(published):
        return published.get_span_context()  # type: ignore[union-attr]
